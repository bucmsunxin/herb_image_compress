...
def make_data_sampler(dataset, shuffle, distributed, is_train, cfg):
    if is_train:
        return samplers.ClassSampler(dataset, cfg)
    # if distributed:
    #     return samplers.DistributedSampler(dataset, cfg, shuffle=shuffle, is_train=is_train)
    if shuffle:
...
def make_batch_data_sampler(
    dataset, sampler, images_per_batch, num_iters=None, start_iter=0, is_train=True
):
...
def make_data_loader(cfg, is_train=True, is_distributed=False, start_iter=0):
    ...
    if is_train:
        ...
        if cfg.MODEL.CUT.IS_OPEN:
            num_iters = cfg.MODEL.CUT.CUT_ITERS
            shuffle = True
        else:
            num_iters = None
            shuffle = False
